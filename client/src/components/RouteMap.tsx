...
type Props = { destination: Destination; activities: Activity[]; selectedId?: string; onSelect?: (id: string) => void; compact?: boolean; isFallback?: boolean };
export default function RouteMap({ destination, activities, selectedId, onSelect, compact = false, isFallback = false }: Props) {
  ...
  return <section className={`relative overflow-hidden rounded-[2rem] border border-[#123a35]/10 bg-[#dceae1] ${compact ? "min-h-[290px]" : "min-h-[520px]"}`} aria-label={`Stylized map of ${destination.name}`}>
    ...
    <div className="absolute left-5 top-5 flex items-center gap-2 rounded-full border border-[#123a35]/10 bg-[#fffdf8]/90 px-3 py-2 text-xs font-bold text-[#123a35] shadow-sm"><MapPinned className="h-3.5 w-3.5 text-[#e6651b]" /> {isFallback ? "Route sketch · planning estimate" : destination.mapLabel}</div>
